...
test("C-08 preference set freezes after scenario generation",async()=>{
 ...
 const c=new Client({connectionString:process.env.DATABASE_URL});await c.connect();await c.query("UPDATE optimisation_preference SET frozen_at=now() WHERE risk_profile_version_id=$1",[p.versionId]);await c.query("INSERT INTO scenario(scenario_id,risk_profile_version_id,optimisation_preference_id,generation_version,generated_at,preference_snapshot_json,generation_fingerprint,generation_ordinal,status) VALUES($1,$2,$3,$4,now(),$5::jsonb,$6,1,'GENERATED')",["SCN-C08-FROZEN",p.versionId,saved.items[0].preferenceId,"sp2-gen-v1",JSON.stringify({voluntary_excess:250}),"c08-freeze-test"]);await c.end();
