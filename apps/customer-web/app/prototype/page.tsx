...
export default function PrototypeEntry(){
  async function start(){const r=await fetch(`${API_URL}/profiles`,{method:"POST"});if(!r.ok)throw new Error("create profile failed");const p=await r.json();location.href=`/profile/${p.profileId}/section/identity`;}
