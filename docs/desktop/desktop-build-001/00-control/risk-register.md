...
DB-G0 inherits every PREP risk below, by original ID and verbatim state/control, from f01776e9bb98bb47a0693f43194d1d48b03b6d5e. Closed PREP items remain historical; all OPEN, CONTROLLED, ACCEPTED and proof/implementation-pending items remain carried. Historical gateway labels do not grant new BUILD authority or close later product proof obligations. G9 closeout governs completed PREP certification.

BUILD reviews these risks at the applicable gateway. In particular: R-G2-004 unsupported resources remain deferred; R-G3-001/002, R-G5-001/003 and R-G6-001 retain external/platform dependencies; R-G4-001 and R-G7-006/010 retain signing gates; R-G7-008 retains the inherited four moderate findings without an unapproved dependency update. Current DB-G0 is documentation-only and does not accept a new security/integrity exception.

## New BUILD risks

| ID | Risk | State | Control |
|---|---|---|---|
| R-DB-G0-001 | UX demo customer controls could be mistaken for Admin authority. | CONTROLLED | Visual-only reference; initial G2 register; formal adoption deferred to DB-G1. |
| R-DB-G0-002 | Control scaffolding could be mistaken for completed later gateway work. | CONTROLLED | Explicit NOT STARTED records; no future PASS or application changes. |
| R-DB-G0-003 | Local general-purpose runtime differs from certified build toolchain. | CONTROLLED | Local checks are documentation/pin/boundary only; repository CI retains exact Node/npm; no local package claim. |

## Inherited PREP snapshot
### PREP Risk Register
...
| R-G1-001 | Tauri introduces Rust/MSVC into a Node/npm repository. | CLOSED BY G8 | G4/G7/G8 | Same-head G7/G8 Windows runs pass fmt, Clippy, tests, package build and installed proof. |
...
| R-G4-005 | Packaging design has not yet been executed against the real Desktop scaffold. | CLOSED BY G8 | G7/G8 | Real NSIS install, launch, upgrade, downgrade rejection and uninstall passed in run `35736586909`. |
...
| R-G7-009 | Released tauri-build 2.6.3 rejected build.windows.staticVCRuntime in the scaffold config. | CLOSED | G7-B | Authorised narrow compatibility correction; later full package CI passed. |
| R-G7-010 | G7 mechanical package is unsigned. | OPEN / CONTROLLED | G7/G9 | Manifest records Authenticode=NotSigned; acceptable only for controlled mechanical proof. Production signing requires UI-SIGN before production release certification. |
| R-G7-011 | PR workflow package provenance records GitHub synthetic merge SHA rather than branch head SHA. | CONTROLLED | G7 | Merge commit explicitly pairs Desktop head 65d1b7b... with frozen base ce211bf...; evidence records both revisions. |
| R-G8-001 | Installed proof harness assumes every uninstall registry object exposes DisplayName under PowerShell strict mode. | CLOSED | G8 | Safe property-existence filtering passed in G8 run `35736586909`. |
| R-G8-002 | Silent NSIS execution could bypass the framework downgrade check. | CLOSED BY G8 | G8 | Pre-install hook independently rejects older semantic versions; installed proof confirms 0.1.0 cannot replace 0.1.1. |
