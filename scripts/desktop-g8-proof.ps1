...
try {
  ...
  Copy-Item $upgradeInstaller.FullName (Join-Path $ProofDir $upgradeInstaller.Name) -Force
} finally {
  git checkout -- $packagePath $tauriConfigPath $cargoPath $cargoLockPath
}

$upgradeInstallerPath = (Get-ChildItem $ProofDir -Filter "*0.1.1*x64-setup.exe" -File | Select-Object -First 1).FullName
$upgradeExit = Invoke-Executable $upgradeInstallerPath @("/S")
